...
const HIWCard = ({ imageSrc, title, description, imgBg, index, btnCls, position }: HIWCardProps) => {
     ...
     return (
          <div
               className={cn(
                    "h-[280px] w-full rounded-[20px] shadow-[0px_4px_17.8px_rgba(0,0,0,0.15)] bg-white", position
               )}>
               <div className="relative h-full w-full p-5 flex items-center justify-center lg:block">
                    <div>
                         <div
                    ...
                    <div className="absolute w-[30px] h-[28px] left-1/2 bottom-0 translate-y-1 -translate-x-1/2 bg-white rounded rotate-45 flex-none order-2 flex-grow-0 z-20 hidden lg:block"></div>
                    <div className="absolute h-fit w-fit left-1/2 -translate-x-1/2 -top-5 lg:top-auto lg:-bottom-16">
                         <ButtonCircle
                              text={index.toString()}
                              disabled={true}
                              className={btnCls ?? ""}
